-- Public catalogue baseline (versioned migration).
-- Production-safe replacement for running supabase seed on RankTiger PROD.
-- Generated from the brand-safe shared catalogue seed at Patch 5.1.
-- Idempotent upserts only; contains no users, questions, tests, attempts, payments, or product identity keys.
