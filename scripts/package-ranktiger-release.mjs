...
function normalizeVersion(value) {
  ...
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$/.test(version)) {
    fail('RANKTIGER_RELEASE_VERSION must be SemVer, for example 1.2.1 or 1.2.1-rc.2.');
  }
  return version;
}
...
async function assertRankTigerDist() {
  ...
  const requiredHtml = ['index.html', 'reset-password.html', 'student.html', 'admin.html', 'test-builder.html'];
...
if (
  ...
  fail('RankTiger release policy is not in the approved 32-migration RankTiger 1.2.1 candidate-only safe mode.');
}
...
if (
  migrationLock.lock_version !== 'RANKTIGER_32'
  || migrationLock.migration_count !== policy.requiredMigrationCount
  || lockedMigrationNames.length !== policy.requiredMigrationCount
) {
  fail('RankTiger migration lock metadata does not match the approved 32-migration RankTiger 1.2.1 baseline.');
}
